...
class ModelPush : public ModelPlugin
{
  ...
  public:
    void JointEffortCallback(const std_msgs::Float32MultiArray::ConstPtr &msg)
    {
        ...
        for (int i = 0; i < msg->data.size(); i++)
            this->pastCommand.push_back(msg->data[i]);
    }
  ...
  public:
    void OnUpdate(const common::UpdateInfo &_info) /*_info*/
    {
        ...
        for (int i = 1; i < joints.size(); i++)
        {
            ...
            joint_state.position[i - 1] = joints[i]->Position(0);
            joint_state.velocity[i - 1] = joints[i]->GetVelocity(0);
            // TODO: Implement this 
            // joint_state.effort[i - 1] = joints[i]->GetForce(0);
        }
